...
$allEssayResultsOpt = $pdo->query("
    ...
    FROM (
        SELECT * FROM (
            SELECT *, ROW_NUMBER() OVER(PARTITION BY exam_id ORDER BY created_at DESC) as rn
            FROM essay_results
        ) WHERE rn <= 500
        UNION ALL
        SELECT *, 0 as rn FROM essay_results WHERE is_graded = 0 AND id NOT IN (
            SELECT id FROM (
                SELECT id, ROW_NUMBER() OVER(PARTITION BY exam_id ORDER BY created_at DESC) as rn
                FROM essay_results
            ) WHERE rn <= 500
        )
    ) as r
...
foreach ($allEssayResultsOpt as $res) {
    ...
    if ($res['rn'] > 0 && $res['rn'] <= 500) {
        $essayResultsByExamId[$res['exam_id']][] = $res;
    }
}
$endOpt = microtime(true);
echo "Window function query time: " . ($endOpt - $startOpt) . " seconds.\n";
