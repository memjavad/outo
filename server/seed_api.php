...
if ($action === 'seed_chunk') {
    ...
    if (strpos($targetJson, 'http') === 0) {
        ...
        if (!$parsedUrl || !isset($parsedUrl['host']) || $parsedUrl['host'] !== 's.nabuo.org') {
            echo json_encode(['status' => 'error', 'message' => "Untrusted domain for remote payload"]);
            exit;
        }
        ...
        if (!file_exists($jsonFilePath)) {
            echo json_encode(['status' => 'error', 'message' => "Local missing payload: " . basename($jsonFilePath)]);
            exit;
        }
