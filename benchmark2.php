...
// Since essay submissions view requires `$essayResultsByExamId`, but we could just do a LIMIT 500 per exam, or no LIMIT but sort them?
// Actually, it might be better to just pull all essay results in SQL directly grouped or grouped in PHP but ordered. Wait.
...
foreach ($allEssayResultsOpt as $res) {
    ...
    // Limit to top 200 per exam to prevent UI bloat
    if (!isset($essayResultsByExamId[$res['exam_id']])) {
        $essayResultsByExamId[$res['exam_id']] = [];
    }
    if (count($essayResultsByExamId[$res['exam_id']]) < 200) {
        $essayResultsByExamId[$res['exam_id']][] = $res;
    }
}
$endOpt = microtime(true);
echo "Optimized group time: " . ($endOpt - $startOpt) . " seconds.\n";
