  ...
  Future<void> _validateAndStartExam() async {
          ...
          if (now.isBefore(start)) {
            ...
            if (mounted) {
              ScaffoldMessenger.of(context).showSnackBar(
                SnackBar(
                  content: Text(l10n.examNotStarted(format.format(start))),
        ...
        try {
          ...
          if (now.isAfter(end)) {
            ...
            if (mounted) {
              ScaffoldMessenger.of(context).showSnackBar(
                SnackBar(
                  content: Text(l10n.examEnded(format.format(end))),
                  duration: const Duration(seconds: 4),
                ),
              );
              setState(() => _isStarting = false);
      ...
      try {
        ...
        for (var interface in interfaces) {
          if (interface.name.toLowerCase().contains("tun") ||
              ...
              interface.name.toLowerCase().contains("pptp")) {
            vpnActive = true;
            break;
          }
        }
      ...
      if (vpnActive) {
        if (mounted) {
          ScaffoldMessenger.of(
            context,
          ).showSnackBar(SnackBar(content: Text(l10n.vpnDetected)));
          setState(() => _isStarting = false);
        }
        return;
      }
    ...
    if (exam.requireBiometrics && isMobilePlatform) {
      ...
      try {
        authenticated = await auth.authenticate(
          localizedReason: 'Let OS verify your identity to start the exam',
        );
      } catch (e) {
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
    ...
    if (exam.requireGps) {
      try {
        ...
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(content: Text('GPS required: ${e.toString()}')),
          );
          setState(() => _isStarting = false);
        }
        return;
                  ...
                  children: [
                    ...
                    Text(
                      exam.examType == 'campaign'
                          ...
                          : exam.examType == 'essay'
                          ? (l10n.localeName == 'ar'
                              ? 'معايير التقييم والوقت'
                              : 'Evaluation Criteria & Time Limit')
                    ...
                    if (exam.examType == 'campaign') ...[
                      Builder(
                        builder: (context) {
                          ...
                          if (levelMatch > 150)
                            fuseSeconds = 10;
                          else if (levelMatch > 100)
                            fuseSeconds = 15;
                          else if (levelMatch > 50)
                            fuseSeconds = 20;
                          else if (levelMatch > 25)
                            fuseSeconds = 25;
                            ...
                            children: [
                              _buildRuleItem(
                                ...
                                l10n.localeName == 'ar'
                                    ...
                                    : 'Burning Fuse Timer',
                                l10n.localeName == 'ar'
                                    ? 'لديك $fuseSeconds ثانية صعبة لكل سؤال في هذا المستوى.'
                                    : 'You have exactly $fuseSeconds seconds to survive each question in this level.',
                                Colors.deepOrange,
                              ),
                              _buildRuleItem(
                                LucideIcons.star,
                                l10n.localeName == 'ar'
                                    ...
                                    : '3-Star Mastery',
                                l10n.localeName == 'ar'
                                    ? 'حقق نسبة ٩٠٪ للنجوم الثلاثة. السقوط تحت ٥٠٪ يعني الإقصاء المستمر.'
                                    : 'Achieve 90% accuracy for 3 stars. Falling below 50% means instant failure.',
                                Colors.amber,
                              ),
                              _buildRuleItem(
                                LucideIcons.zap,
                                l10n.localeName == 'ar'
                                    ...
                                    : 'Combo Multiplier',
                                l10n.localeName == 'ar'
                                    ? 'سلسلة الإجابات الصحيحة تضرب مجموع نقاطك لتصل إلى أعلى القمة!'
                                    : 'Stringing correct answers together triples your score pushing you up the Leaderboard!',
                                Colors.purpleAccent,
                              ),
                            ],
                          );
                      ...
                      _buildRuleItem(
                        ...
                        exam.examType == 'essay'
                            ? (l10n.localeName == 'ar'
                                ? 'لديك ${exam.examTimerMinutes} دقيقة للإجابة.'
                                : 'You have ${exam.examTimerMinutes} minutes to complete this essay.')
                            : l10n.minutesTotalLimit(
                              exam.examTimerMinutes.toString(),
                            ),
                        Colors.orange,
                      ),
                    ],
                    ...
                    if (exam.examType == 'essay') ...[
                      _buildRuleItem(
                        ...
                        l10n.localeName == 'ar'
                            ...
                            : 'Grammar & Mechanics',
                        l10n.localeName == 'ar'
                            ? 'سيتم التقييم على النحو النحوي والإملائي وعلامات الترقيم.'
                            : 'Evaluated on syntax, spelling, punctuation, and structural mechanics.',
                        Colors.blue,
                      ),
                      _buildRuleItem(
                        ...
                        l10n.localeName == 'ar'
                            ...
                            : 'Content & Knowledge',
                        l10n.localeName == 'ar'
                            ? 'التركيز على دقة وعمق المعلومات ومدى ارتباطها بالموضوع.'
                            : 'Focused on accuracy, depth of information, and relevance to the central topic.',
                        Colors.indigo,
                      ),
                      _buildRuleItem(
                        LucideIcons.layers,
                        l10n.localeName == 'ar'
                            ...
                            : 'Organization',
                        l10n.localeName == 'ar'
                            ? 'التقييم استناداً إلى تسلسل الأفكار وبناء الفقرات.'
                            : 'Evaluated on logical flow, paragraph structure, and clear transitions.',
                        Colors.teal,
                      ),
                      _buildRuleItem(
                        ...
                        l10n.localeName == 'ar'
                            ? 'الأصالة في الطرح، الرؤية الشخصية، والأسلوب المتميز.'
                            : 'Originality in thought, personal perspective, and engaging voice.',
                        Colors.purple,
                      ),
                    ] else ...[
                      if (exam.questionTimerSeconds > 0)
                        _buildRuleItem(
                          ...
                          l10n.secondsPerQuestionLimit(
                            exam.questionTimerSeconds.toString(),
                          ),
                          Colors.deepOrange,
                        ),
                    ...
                    if (exam.recordScreen || exam.preventScreenshots)
                      _buildRuleItem(
                        ...
                        exam.recordScreen
                            ? l10n.screenRecordDesc
                            : l10n.preventScreenshotsDesc,
                        Colors.indigo,
                      ),
                  ],
                ),
                ...
                child: ElevatedButton.icon(
                  ...
                  label: Text(
                    _isStarting
                        ...
                        : exam.examType == 'campaign'
                        ? (l10n.localeName == 'ar'
                            ? 'أنا مستعد. ابدأ التحدي'
                            : 'I am Ready. Begin Level')
                        : (exam.examType == 'essay'
                            ? (l10n.localeName == 'ar'
                                ? 'أوافق. ابدأ الواجب'
                                : 'I Accept. Begin Essay')
                            : l10n.acceptBeginExam),
                    style: const TextStyle(
                      fontSize: 18,
                      fontWeight: FontWeight.bold,
                    ),
