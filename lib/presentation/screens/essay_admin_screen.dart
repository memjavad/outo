...
class _EssayAdminScreenState extends State<EssayAdminScreen>
  ...
  Future<void> _loadData() async {
    ...
    if (mounted) {
      setState(() {
        ...
        _pendingEssays =
            pending.where((p) {
              final e = _essays.firstWhere(
                (x) => x.id == p.examId,
                orElse: () => Exam(id: '', title: 'Unknown', isActive: false),
              );
              return e
                  .id
                  .isNotEmpty; // only include pending results that map to an existing essay
            }).toList();
      ...
      if (success) {
        ...
        if (mounted)
          messenger.showSnackBar(
            const SnackBar(
              content: Text('Essay created successfully'),
    ...
    if (dialogResult == true && scoreController.text.isNotEmpty) {
      ...
      if (success) {
        ...
        if (mounted)
          messenger.showSnackBar(
            const SnackBar(
              content: Text('Essay Graded Successfully!'),
              backgroundColor: Colors.green,
            ),
          );
      } else {
        if (mounted)
          messenger.showSnackBar(
            const SnackBar(
              content: Text('Failed to submit grade!'),
              backgroundColor: Colors.red,
            ),
          );
      }
    ...
    return RefreshIndicator(
      ...
      child: ListView.builder(
        ...
        itemBuilder: (context, index) {
          ...
          final relatedExam = _essays.firstWhere(
            (e) => e.id == result.examId,
            orElse: () => Exam(id: '', title: 'Unknown Exam', isActive: false),
          );
            ...
            child: ListTile(
              ...
              subtitle: Column(
                ...
                children: [
                  ...
                  Text(
                    'Exam: ${relatedExam.title}',
                    style: TextStyle(color: Colors.grey.shade700),
                  ),
                  const SizedBox(height: 2),
                  Text(
                    'Date: ${result.timeTaken.inMinutes} mins taken',
          ...
          Container(
            ...
            child: TabBar(
              ...
              tabs: [
                ...
                Tab(
                  icon:
                      _pendingEssays.isNotEmpty
                          ? Badge(
                            label: Text('${_pendingEssays.length}'),
                            child: const Icon(LucideIcons.inbox),
                          )
